...
class PresensiController extends Controller
{
        ...
        if ($radius > $lok_kantor->radius_cabang) {
            ...
            if ($cek > 0) {
                if ($jam_pulang < $jamkerja_pulang) {
                    ...
                    echo "error | Sudah Melakukan Absen Pulang! | out";
                } else {
                    $data_pulang = [
                ...
                if ($jam < $jamkerja->awal_jam_masuk) {
                    echo "error | Maaf Belum Waktu nya Presensi | in";
                } else if ($jam > $jamkerja->akhir_jam_masuk) {
                    echo "error | Maaf Batas Waktu Presensi Sudah Habis | in";
                } else {
                    $data = [
                        'nik' => $nik,
    ...
    public function getpresensi(Request $request)
    {
        ...
        $query->selectRaw('karyawan.nik,nama_lengkap, karyawan.kode_dept, karyawan.kode_cabang,datapresensi.id,jam_in,jam_out,foto_in,foto_out,lokasi_in,lokasi_out,datapresensi.status,jam_masuk, nama_jam_kerja, jam_pulang, keterangan');
        $query->leftjoin(
            DB::raw("(
                SELECT
    ...
    public function cetaklaporan(Request $request)
    {
        ...
        if (isset($_POST['exportexcel'])) {
            ...
            //Mendifinisikan nama file ekspor "hasil-export.xls"
    ...
    public function accizinsakit(Request $request)
    {
        ...
        $accizinsakit = $query->paginate(10);
        $accizinsakit->appends($request->all());
        return view('presensi.accizinsakit', compact('accizinsakit'));
    }
